...
#     MassLoading <- reactive({
#         (area$For*input$coefF)+(area$Agr*input$coefA)+(area$Urb*input$coefU)+(lake$Ao*input$coefPr)+(NCap*input$coefSt*(1-input$coefSr))+input$coefPSI
#     })
    ...
    output$range <- renderText({
        ...
        paste0("For the lake Higgins there is a ", input$hvar, " probability that the true phosphorus concentration value falls between ", round(limitlo,4), " mg/L (", trophlo, ") and ", round(limitup,4), " mg/L (", trophup, ").")
    })
    ...
    output$laketype <- renderPlot({
        ...
        # multiple of prediction error, h=1=55% bounds, h=2=90% bounds
        c = sqrt((-2.25*input$hvar)+2.25)
        limitlo2 <- PLk-(c*sTneg)
        limitup2 <- PLk+(c*sTpos)
